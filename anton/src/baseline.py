...
class Linear_Baseline:
    def __init__(self, n_lags:int=2, n_regions:int=100, intercept:bool=False):
    ...
    def _prepare_data(self, exog, outcome):
        ...
        X = torch.Tensor(exog.merge(self.seansanal_trends, on=['month', 'day', 'region'], how='left').values)
        y = torch.Tensor(outcome.values)
        
        lagged_outcome = torch.stack([y.roll(self.n_regions*lag) for lag in range(1, self.n_lags+1)]).T
